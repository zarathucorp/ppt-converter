...
export default function Home() {
      ...
      setFiles((prev) => {
        ...
        for (const file of filesToProcess.slice(0, Math.max(availableSlots, 0))) {
          if (file.size > MAX_SINGLE_FILE_SIZE) {
            errorMessage =
              errorMessage ??
              t("conversion.fileTooLarge", { filename: file.name });
            continue;
          }
  ...
  const convertToPptx = async () => {
    ...
    const oversizedFile = files.find(
      (fileObj) => fileObj.file.size > MAX_SINGLE_FILE_SIZE
    );
    if (oversizedFile) {
      setError(
        t("conversion.fileTooLarge", { filename: oversizedFile.file.name })
      );
      return;
    }
    ...
    try {
      const hasComplexSvg = files.some(
        (fileObj) =>
          fileObj.file.name.toLowerCase().endsWith(".svg") &&
          fileObj.file.size > 50000
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="mt-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-4">
                <div className="flex items-center space-x-4">
                  <label
                    htmlFor="filename"
                    className="flex-shrink-0 text-sm font-medium text-gray-700 w-20"
                  >
                    {t("settings.filename")}:
                  </label>
                  <div className="flex-1 flex itemscenter space-x-2">
                    <input
                      type="text"
                      id="filename"
            ...
            <div className="mt-6 flex justify-center">
              <button
                ...
                ${files.length === 0 || isConverting
                  ...
                  : "bg-slate-900 text-white border-slate-900 hover:bg-slate-800 hover:border-slate-700"
                }`}
              >
      ...
      <footer className="bg-gray-100 border-t border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-6 text-center text-sm text-gray-600 space-y-2">
          <p>
            Copyright © 2025{" "}
            <a
          ...
          <p className="text-xs text-gray-500">
            Released under the{" "}
            <a
              href="https://github.com/zarathucorp/ppt-converter/blob/main/LICENSE"
              target="_blank"
              rel="noopener noreferrer"
              className="underline"
            >
